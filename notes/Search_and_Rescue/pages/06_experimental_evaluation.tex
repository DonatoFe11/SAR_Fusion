...
FAM increases mean \mapfifty{} over Additive by 0.0700 and wins in all five
...
\begin{figure}[p]
  \centering
  \thesisimage[\textwidth]{images/rtdetr_final_paired_map50.pdf}{Paired-seed plot for the six final RT-DETR configurations, emphasizing FAM versus Additive and the paired ablation contrasts.}
  \caption{Final RT-DETR VIS+IR \mapfifty{} from the \texttt{latest}
  checkpoints. Panel A connects Additive and FAM runs initialized with the same
...
\Cref{tab:rtdetr-contrasts} compares FAM with Additive and compares each FAM
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    FAM -- Additive & +0.0700 & 0.0531 & [+0.0040,+0.1359] & 5/5 & 0.0421 / 0.0625 \\
...
without SSJ and without IR Dropout. The decision favors the consistent paired
gain over Additive rather than the highest numerical mean.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    Additive & 0.1543 \(\pm\) 0.0331 & 0.1567 \(\pm\) 0.0464 & 0.3081 \(\pm\) 0.0677 \\
...
FAM also improves Additive in VIS-only in 5/5 seeds, with a mean delta of
...
maps are abstract feature projections, not registered RGB and thermal images.
...
Grid Sample has smaller and less dispersed P3/P4 offsets and preserves the
input feature more closely. Its same-cell cosine similarity increases slightly
after the warp. DCNv2 instead reduces same-cell cosine similarity and increases
standardized RMSE because it also filters and mixes channels. This does not
show that DCNv2 worsens physical alignment; it shows that its output is not
merely the IR tensor shifted to resemble RGB at the same cell.
...
\begin{figure}[p]
  \centering
  \thesisimage[0.9\textwidth]{images/fam_p5_seed41_collapse.png}{Mechanistic diagnostic for MtErie sample 35 at P5 in FAM seed 41, showing shared-basis PCA projections, qualitative blends, and offset, mask, and output statistics.}
...
created by the anomaly: after excluding seed 41, FAM still beats Additive in
...
The error analysis compares the five Additive and five standard-FAM final
checkpoints on all 708 MtErie frames and 1,770 objects. Predictions are matched
one-to-one to annotations at IoU at least 0.50. Threshold 0.01, already used to
collect predictions for the final evaluations, is the primary collection
threshold; 0.05, 0.10, 0.25, and 0.50 form a predefined sensitivity analysis.
The analysis does not select a deployment threshold.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{rrcccc}
    ...
    \midrule
    0.01 & Additive & 0.0418 \(\pm\) 0.0175 & 0.7677 \(\pm\) 0.0490 & 51.006 \(\pm\) 21.591 & 0.4508 \(\pm\) 0.0777 \\
    0.01 & FAM & 0.0443 \(\pm\) 0.0132 & 0.8044 \(\pm\) 0.0328 & 47.623 \(\pm\) 17.873 & 0.4009 \(\pm\) 0.0524 \\
    0.05 & Additive & 0.2713 \(\pm\) 0.0909 & 0.5932 \(\pm\) 0.1134 & 4.844 \(\pm\) 3.199 & 0.6476 \(\pm\) 0.1155 \\
    0.05 & FAM & 0.3010 \(\pm\) 0.0940 & 0.6539 \(\pm\) 0.0972 & 4.336 \(\pm\) 2.191 & 0.5835 \(\pm\) 0.0775 \\
    0.10 & Additive & 0.4721 \(\pm\) 0.0923 & 0.4582 \(\pm\) 0.1535 & 1.513 \(\pm\) 1.115 & 0.7466 \(\pm\) 0.1223 \\
    0.10 & FAM & 0.5194 \(\pm\) 0.1148 & 0.5507 \(\pm\) 0.1333 & 1.447 \(\pm\) 0.792 & 0.6554 \(\pm\) 0.0725 \\
    0.25 & Additive & 0.7290 \(\pm\) 0.0866 & 0.2340 \(\pm\) 0.1468 & 0.249 \(\pm\) 0.218 & 0.8682 \(\pm\) 0.0879 \\
    0.25 & FAM & 0.7715 \(\pm\) 0.0742 & 0.3330 \(\pm\) 0.1506 & 0.276 \(\pm\) 0.168 & 0.7858 \(\pm\) 0.0525 \\
    0.50 & Additive & 0.9140 \(\pm\) 0.0536 & 0.1088 \(\pm\) 0.0705 & 0.029 \(\pm\) 0.033 & 0.9257 \(\pm\) 0.0588 \\
...
seeds. The thesis therefore does not claim that FAM reduces false positives.
...
small target and one empty frame. Seed 43 is used because its FAM--Additive
...
\begin{figure}[htbp]
  ...
  The same frames and display rules are used for Additive and FAM. These
...
Additive in one frame; both models miss the targets in the other two. No
...
inference and admits only the five final Additive and five final FAM
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrr}
    \toprule
    Input & Additive & FAM & Mean paired delta & FAM wins \\
    \midrule
...
FAM is more tolerant than Additive in the fusion condition, with a mean gain
...
The compute comparison uses the seed-43 Additive and FAM checkpoints, chosen in
advance as the same typical pair used for qualitative illustrations. Each
...
\begin{table}[htbp]
  \centering
  \caption{Detector-only computational benchmark. The operation count is a
  ...
  \begin{tabular}{lrrrrr}
    ...
    \midrule
    Additive & 66.20 M & 208.37 & 66.91 \(\pm\) 0.30 ms & 14.95 image/s & 436.26 MiB \\
...
Relative to Additive, FAM adds 77.5\% parameters, 46.2\% proxy computation,
...
Additive configuration sums each pair; the FAM configuration applies the same
...
\begin{figure}[p]
  \centering
  \thesisimage[0.92\textwidth]{images/yolov10_dual_backbone_fam.pdf}{Diagram of the implemented YOLOv10 dual backbones, P3/P4/P5 Additive or FAM fusion, FPN/PAN neck, and v10Detect head.}
  ...
  bypassed; consequently, unimodal Additive--FAM differences are indirect
...
The historical seed-42 Additive and FAM checkpoints were selected at epochs 16
...
The final comparison fixes two configurations: dual-backbone Additive and
dual-backbone standard FAM, both with feature gating 20/20/60. Each uses paired
seeds 40--44, an isolated Python process, 200 fixed epochs, disabled early
stopping, no validation-based selection, and the final \texttt{last.pt}
checkpoint. The 200-epoch horizon was retained because it was declared in the
historical YAML schedule before the final results were observed.

All ten runs completed 200 epochs. Their automatic VIS+IR results are shown in
\Cref{tab:yolo-raw}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lrrrr}
    \toprule
    Input & Additive & FAM & Mean delta & FAM wins \\
    \midrule
...
At seed 42, the historical early Additive checkpoint obtained 0.3469 at epoch
16 and the historical FAM checkpoint 0.2965 at epoch 89, compared with 0.2494
and 0.1947 at epoch 200. Additive training losses and learning rate match the
