...
\subsection{Stage-A checkpoint baseline}

The complete-video split and checkpoint rule in
\Cref{sec:two-stage-protocol} were first applied to standard FAM for seeds
40--44. All runs completed ten epochs. The selected epochs were 1, 4, 6, 1,
and 1, showing that the decline after early epochs was repeatable rather than a
logging anomaly.
...
\subsection{Stage-A candidate screening}

\Cref{tab:stage-a-candidates} summarizes the ablations performed under the
common complete-video validation protocol. FAM is the paired reference for
every delta.
...
Stage B restores all 4,019 training frames and trains RCRA and a current-code
FAM baseline for ten epochs, without validation or automatic MtErie testing.
...
final performance baseline. This is not evidence that RCRA is uniformly worse:
...
baseline: matching the paired evaluation convention removes useful native-IR
...
additive fusion and is not a post-selection comparison of RCRA and the
current-code Stage-B baseline.
...
transfer result. A new YOLO study should reuse the complete-video Stage-A split
defined in this thesis and freeze a YOLO-specific checkpoint rule before any
new MtErie evaluation.
