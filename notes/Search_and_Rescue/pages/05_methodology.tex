...
\begin{figure}[p]
  ...
  conducted under the previous protocol. The audit motivates the redesign; the
  horizontal separator marks the transition to the locked RT-DETR and YOLOv10
  multi-seed evaluations. Vertical placement indicates narrative order, not
  elapsed time.}
...
\begin{table}[htbp]
  \centering
  \caption{Historical single-run FAM development results. These values belong
  to the exploratory phase and do not determine the final model selection.}
...
The Computer Vision project had already integrated FAM before the channel
fusion block. The post-coursework extension added SSJ under the same previous
...
\begin{figure}[htbp]
  ...
  \caption{Deformable DETR dual-backbone fusion architecture. The fourth
  encoder level is derived from the fused deepest native feature and does not
  receive a separate fourth FAM. This diagram documents an exploratory line
  that precedes the final RT-DETR protocol.}
  \label{fig:defdetr-fusion}
\end{figure}
...
SSJ does not reproduce the gain suggested by its historical RT-DETR run and
...
base of Deformable DETR and activates all three defining mechanisms. MQS uses
the top encoder proposals as spatial anchors while drawing query content from
an independent learned embedding. LFT keeps the refined reference passed to
the next attention layer detached while preserving a graph-connected copy for
the following prediction loss. CDN inserts noisy positive and negative target
queries during training and applies the required decoder attention mask.

Earlier runs labeled ``DINO'' in project reports were a CDN legacy variant:
box refinement was disabled, MQS was absent, and the CDN mask did not reach the
decoder self-attention. They are not ablations of the complete model and are
excluded from the main conclusion.

The complete base was trained five times and produced the values in
...
\begin{table}[htbp]
  \centering
  \caption{Complete DINO base result under the previous protocol, reported as
  ...
  \begin{tabular}{lccc}
    ...
    Configuration & VIS & IR & VIS+IR \\
    \midrule
    Complete DINO base & 0.0565 [0.0232--0.1091] & 0.0881 [0.0671--0.1473] & 0.1088 [0.0669--0.1635] \\
    \bottomrule
  \end{tabular}
\end{table}

This is a negative result for the implemented configuration and training
budget. It does not show that DINO is intrinsically unsuitable for WiSARD.
...
regression-tested so that dropout is active only in training. All final
...
checkpoint. Validation is not run as a selector. This does not replace the need
for a future representative train-derived validation set.

\subsection{Process isolation}

Sequential grids in one Python process showed progressive slowdowns and could
retain allocator or framework state. Each seed in the final campaigns is
therefore launched in a new process. The same principle is applied to compute
benchmark trials, where an initial in-process memory result was rejected after
cumulative allocations were detected.
...
architectural contrasts. It does not make native executions deterministic.
