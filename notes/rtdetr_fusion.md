...
Inizializza il modulo definendo i pesi che dovrà imparare.
...
```python
    ...
    self.fam_modules = None if use_fam else False
```
**Inizializzazione Lazy (Pigra)**: I moduli FAM vengono dichiarati `None` (non ancora creati). Perché? Perché le feature map in uscita dai vari livelli della backbone hanno numeri di canali diversi. Piuttosto che inserirli a mano rischiando errori, la rete aspetterà che passi il primo batch di immagini (nel `forward`) per guardare quanti canali hanno effettivamente i tensori e instanzierà i FAM con le dimensioni corrette automaticamente.
...
```
Ecco l'attivazione *lazy* all'opera. Scorre le feature map ($C3, C4, C5$) appena estratte e per ognuna instanzia un modulo FAM passandogli il corretto numero di canali di quel livello (512, 1024, o 2048). Li raggruppa tutti in una `nn.ModuleList`.
