...
\begin{longtable}{p{0.38\textwidth}p{0.54\textwidth}}
  ...
  \path{results/rtdetr_recall_fppi_summary.csv} & Nine population--budget summaries of historical Base and FAM recall, paired differences, and wins. \\
  \path{results/rtdetr_recall_fppi_budgets.csv} & Ninety checkpoint--budget points, including attained FPPI, descriptive confidence thresholds, TP, and FP. \\
  \path{results/rtdetr_recall_fppi_paired.csv} & Forty-five within-seed recall differences; repeated budgets and populations are not independent replications. \\
  \path{results/rtdetr_recall_fppi_grid.csv} & Mean and sample SD on the common 301-point FPPI grid for each configuration and population. \\
  \path{results/rtdetr_recall_fppi.json} & Complete 30-unit protocol, checkpoint and inventory hashes, software provenance, and cache references. \\
  ...
  \path{images/rtdetr_recall_fppi.pdf} & Recall--FPPI curves for historical Base and FAM on MtErie, Carnation 0025/0026, and FHL 0407/0408. \\
...
\path{scripts/generate_thesis_figures.py}. The recall--FPPI figure and compact
tables are instead regenerated from the complete local prediction caches with
\path{scripts/run_rtdetr_recall_fppi.py} using \texttt{--summarize-only}; this
does not require new inference. Historical public
