...
Does FAM provide a repeatable advantage over the Base RT-DETR configuration,
which uses direct additive fusion, and does the selected detector obtain a
measurable benefit from the thermal stream when all modality conditions are
evaluated on the same paired frames and ground truth?
...
modulation of the FAM correction improve it more consistently than simpler
...
under the implemented Deformable DETR, YOLOv10, RT-DETRv2, and YOLO26
configurations?

\section{Scope and relationship with previous work}

This thesis builds on preliminary RGB--thermal detection project work conducted
before this thesis. That work examined DETR, Modal Dropout, the Base RT-DETR
configuration with direct additive fusion, fixed tiling, and CMX. Subsequent
preliminary work introduced FAM on RT-DETR, developed a Hybrid CMX variant, and
adapted Deformable DETR to channel fusion with FAM. These components, preserved
in the public project repository identified in \Cref{chap:appendices}, define
...
Model comparisons are reported within their respective data splits, training
budgets and checkpoint-selection protocols. Analysis of FAM's internal
behavior complements these comparisons, but does not replace the performance
criteria used for Stage-A/B model selection.
...
\begin{enumerate}[label=\arabic*.]
  ...
  \item a five-seed validation-led comparison of tiny-object, resolution,
        offset-initialization, reliability, residual-alignment,
        mixed-consistency, and supervised box-guided hypotheses under the
        common Stage-A/B protocol, including
        configuration-matched full-data confirmation of RCRA and FAM;
  ...
  \item an explicit delimitation of the result through paired error analysis
        and recall at matched false-positive budgets,
  ...
  \item a predeclared and versioned confirmation on two previously unused
        internal WiSARD acquisitions and a controlled synthetic geometric
        stress test;
  \item paired five-seed Stage-A transfer comparisons of Base and FAM
        fusion in RT-DETRv2 and YOLO26, with validation-selected checkpoints
        as primary endpoints and final-epoch checkpoints as diagnostics.
\end{enumerate}
...
point. \Cref{chap:methodology} defines the experimental methods, distinguishes
the historical locked comparison from validation-led model development, and
specifies the common five-seed, two-stage protocol.
...
confirmation on previously unused acquisitions, controlled geometric stress,
computational cost, and five-seed detector-transfer studies.
Finally, \Cref{chap:discussion} answers the research questions, states the validity
